...
The byte unpacker installs a 512-item table and performs two checked queries
per byte. Its representative 16-byte fragment is measured below; its strict
standalone ceiling is 243 bytes because two output nibbles per processed byte
accumulate on altstack while the shared table remains live.
...
their order is operation-specific. The unpacker consumes one hostile byte data
item per invocation and has zero incremental hint items; its 16-byte metric
uses 49 serialized witness bytes. Tables are generated by the locking script.
...
For `unpack::u4_bytes_to_nibbles(n)`, input is
`preserved | byte[0] | ... | byte[n-1]`, with `byte[n-1]` on top. Each byte is
range-checked as `0..=255` and replaced by `high_nibble | low_nibble`; the
final low nibble is on top. The generator rejects empty batches and batches
above 243. The standalone strict peak is `512 + 2*n` after decoding, with up
to three transient items for the range check; unrelated live state requires a
smaller batch.
