...
item with the item at a caller-selected wrapped offset, and returns one
ScriptNum bit per input. It is useful for periodicity checks and keeps the
vector width unchanged, unlike a non-wrapped adjacent-pair mask.
...
Cyclic equality range-checks both copies used by every comparison. Its output
is a ScriptNum bit and does not authenticate the vector length or provide a
terminal predicate.
...
standalone batch under the strict local stack limit. `cyclic_equality.rs`
rejects malformed inputs and invalid batch sizes and measures a representative
strict batch; `parity.rs` exhaustively checks its 16-value lookup domain and
measures a representative strict batch.
