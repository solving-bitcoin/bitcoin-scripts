//! Checked bitwise OR of a byte-oriented u32 word and an embedded mask.
...
/// ORs the top canonical u32 word with an embedded public mask.
///
...
pub fn u32_or_constant(value: u32) -> Script {
    script! {
        for _ in 0..4 {
...
mod tests {
    ...
    fn masks_boundaries_and_patterns() {
        for (word, mask) in [
            ...
            assert!(
                result.success,
                "constant OR failed: {word:08x}|{mask:08x}: {result}"
            );
        }
    }
    ...
    fn preserves_surrounding_main_and_alt_stack_items() {
        ...
        assert!(
            result.success,
            "constant OR did not preserve stack state: {result}"
        );
    }
}
