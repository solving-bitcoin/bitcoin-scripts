//! Checked per-byte equality mask for two u32 byte words.
...
/// Compare corresponding MSB-first bytes and return a four-bit equality mask.
///
/// The top two u32 words are consumed. Bit 3 of the result describes byte 0
/// (the most-significant byte), and bit 0 describes byte 3. Every input byte
/// is checked for range and canonical Script-number encoding before comparison.
pub fn u32_byte_eq_mask() -> Script {
    script! {
        { u32_zip(0, 1) }
        for _ in 0..4 {
            { compare_byte_pair() }
        ...
        // The equality results are now e0 | e1 | e2 | e3, with e3 on top.
...
mod tests {
    ...
    fn matches_boundary_and_random_words() {
        for (a, b) in [
            ...
            (0x1122_3344, 0x1122_3344),
        ] {
            check(a, b);
        }

        let mut rng = ChaCha20Rng::seed_from_u64(0x7533_3265_716d_6173);
        for _ in 0..256 {
