#!/usr/bin/env python3
"""Full honest anchored publication on isolated Core; general extraction is open.
...
def straight_line_stack_peak(script, entry):
    ...
    delta = {0x6d: -2, 0x75: -1, 0x76: 1, 0x78: 1, 0x7a: -1, 0x7c: 0, 0x7d: 1,
             0x82: 1, 0x88: -2, 0x69: -1, 0xa5: -2, 0xa9: 0, 0xab: 0, 0xad: -2}
...
def recover(funding, spend, built):
    ...
    for pool in built['pools']:
        ...
        for slot in range(pool['t']):
            frame = stream[slot * (3 + pool['rounds']):(slot + 1) * (3 + pool['rounds'])]
            tau, hint, key_bytes, *shorts = frame
            ...
            for j, sigma in enumerate(shorts):
                ...
                msg = digest(spend, index, contexts[check + 1 + j], flag)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='bitcoin-lab-anchored-publication-') as temporary:
        ...
        try:
            ...
            command = ['cargo', 'run', '--release', '--locked', '--example', 'pointlock_anchored_publication_probe',
                       '--', '--funding-txid', granted['txid'], '--funding-vout', '0', '--funding-amount', str(amount)]
            result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True, check=True)
            built = json.loads(result.stdout)
            (HERE/'anchored-publication-transactions.json').write_text(json.dumps(built, indent=2)+'\n')
            ...
            node.close()
    (HERE/'anchored_publication_core_check.json').write_text(json.dumps(report, indent=2)+'\n')
