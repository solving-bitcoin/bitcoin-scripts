...
| --- | ---: | ---: | ---: |
| One ordinary multiplication | 20,524 | 94 bytes / 67 items | 757 |
| Raw ordinary two-operand multiplication | 21,800 | 160 bytes / 125 complete items | 757 |
| One factor-16 multiplication | 20,501 | 37 bytes / 29 items | 719 |
| Raw factor-16 two-operand multiplication | 21,777 | 103 bytes / 87 complete items | 719 |
| Two preloaded ordinary multiplications | 39,400 | 187 bytes / 134 items | 882 |
| Three preloaded ordinary multiplications | 59,163 | 280 bytes / 201 items | 993 |
...
of table cleanup. The other 18,729 bytes are product, normalization,
recombination, exact-relation, routing, and output-validation work. Its exact
breakdown is 9,374 low/high product bytes, 5,008 normalized-difference product
bytes, 1,103 normalization bytes, 173 coefficient-routing bytes, 532
recombination bytes, and 2,539 relation/output bytes. The 1,795-byte table
lifecycle is 8.7% of an isolated gate.

The factor-16 gate uses the same 1,795-byte table lifecycle and 18,706 bytes of
computation. Its exact categories are 15,615 product-generation bytes, 2,674
folded-relation bytes, and 417 cleanup/output-certification bytes. It is only
23 locking-script bytes smaller than the ordinary gate, but removes 38 hints
...
returns a reusable certificate. The 20,524-byte ordinary native gate is 10,757
...
fan-out are not included in either fragment. The 20,501-byte factor-16 gate is
an additional 23 bytes smaller and uses only 29 hints, but is comparable only
...
degree-28 exact relation and measures 20,501 bytes. These are executable search
