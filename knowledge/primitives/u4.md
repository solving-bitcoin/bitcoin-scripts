...
- **Representative results:** addition-table setup is 92 script bytes. A
  ...
  1,374 bytes and peaks at 130. The checked 16-byte unpacker is 1,168 bytes,
  executes 544 non-push opcodes, and peaks at 546 combined stack items with a
  49-byte, 16-item witness.
...
- **Input boundary:** the checked decomposition proves numeric range `0..=15`;
  byte unpacking proves `0..=255`; unchecked lookup requires previously
  certified nibbles and neither form alone proves byte-unique ScriptNum
  encoding.
