# Checked u4 trailing-zero projection

`arithmetic::u4::trailing_zeros::u4_nibbles_to_trailing_zeros` consumes a
contiguous batch of numeric nibbles, proves each value is in `0..=15`, and
replaces it with the number of zero bits following its least significant set
bit. Zero maps to four, so the output is always in `0..=4`.

- **Input:** `preserved | nibble[0] | ... | nibble[n-1]`, with the last nibble
  on top.
- **Output:** `preserved | tz[0] | ... | tz[n-1]`, with the last count on top.
...
- **Question:** can a compact lookup expose low-bit alignment classes for
  nibbles without expanding them into four stack items or reversing their
  bits first?
- **Hypothesis:** direct trailing-zero projection has the same byte and stack
  profile as other checked one-output u4 projections while supporting suffix
  alignment and power-of-two divisibility checks.
- **Comparison objective:** compare the direct lookup with LSB projection,
  leading-zero projection, and a bit-reversal-plus-leading-zero composition.
  The direct form retains the full zero case and gives one numeric class per
  input.
...
`u4_nibbles_to_trailing_zeros(32)` includes the generated 16-item table, 32
...
```sh
CARGO_INCREMENTAL=0 cargo test --locked arithmetic::u4::trailing_zeros --lib
CARGO_INCREMENTAL=0 cargo test --locked --test primitive_metrics u4_trailing_zeros_metrics_are_current
