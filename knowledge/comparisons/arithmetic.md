...
| Hinted secp256k1 modular multiply, standalone-bound | 36-prime limb-bound exact-carry verifier | 88,225 | 722-byte complete data witness; 249-item strict peak; global bindings included |
...
Both hinted modular rows are `locally-reproduced` and `unclassified`. The
...
The 36-prime row includes that missing work. Its 244 witness items are 64
centered base-`2^16` limbs, 144 residue-binding carries, and 36 relation
...
and checks the product over a 521-bit basis. It needs no complement. Its 88,225
bytes split into 1,060 bytes of range checks, 75,732 of residue binding,
11,121 of modular relations, and 312 of routing and output. The 722-byte
witness covers all 244 consumed data items for `(N-1)^2`, not merely the
derived carries. The fragment returns both the 16 remainder limbs and 36
residues. A reusable one-value binding costs 19,147 bytes when a larger program
...
otherwise-unchecked field value costs 19,234 bytes.

These two sizes are not a direct optimization comparison: 10,952 excludes the
global proof that dominates the 88,225-byte standalone fragment. The 75-prime
...
Both carry verifiers are table-free. All 10,952 bytes of the conditional
fragment and all 88,225 bytes of the standalone fragment are arithmetic,
validation, binding, and routing rather than reusable lookup setup.
