...
There is no universal winner. The legacy list-pick fragment remains smallest,
but its clamp does not bind the witness's raw above-range numeric value. The
Fast strict lookup profile spends 142 more script bytes for explicit range
validation. The Fast exact profile spends another 402 bytes and reduces
balanced-vector HASH160 calls by 31.2%. Its 5,452-byte script is 126 bytes
smaller than the legacy 5,578-byte binary-search verifier, which targets the
same exact chain suffix.
