...
| --- | ---: | ---: | ---: | --- |
| Centered constant chain | 0 | 129 | peak 4 | Isolated known constant |
| Half direct table, batch 8 | included in 571 | included | peak 139 | Repeated same constant |
| Full direct table, batch 8 | included in 808 | included | peak 266 | Larger same-constant batches |
| Log/exp | 1,004 | 44 constant / 60 variable | 385 memory; peak 900 at depth | Repeated mixed products |
| Exact square | 435 | 11 | 129 memory; peak 643 at depth | Repeated centered squares |
...
| --- | ---: | ---: | --- |
| Existing four-bit branch splitter | 1,374 | 130 | Repeated split-to-altstack plus restoration |
...
| --- | ---: | ---: | ---: | ---: |
| One ordinary normalized-Karatsuba multiply | 20,503 | 1,795 | 18,708 | 757 |
| One factor-16 folded multiply | 20,450 | 1,795 | 18,655 | 719 |
| Two preloaded ordinary multiplies | 39,358 | 1,795 | 37,563 | 882 |
| Three preloaded ordinary multiplies, destructive layout | 59,145 | 1,795 | 57,350 | 993 |
| One specialized square | 14,541 | 1,795 | 12,746 | 614 |
| Five preloaded squares | 65,064 | 1,914 | 63,150 | 998 |

The 513-entry quarter-square table represents about 8.8% of one multiplication,
...
| --- | ---: | ---: | --- |
| Legacy direct-table RNS | 1,561 | 903 | Includes full memory and cleanup |
| 256-bit-product prime hybrid | 15,626 | 183 | Per-coordinate shortest of streamed lookup and centered/plain binary Horner |
...
| Conditional exact-carry modular verifier | 10,950 | 231 | Table-free; external global bindings excluded |
| Standalone-bound exact-carry modular verifier | 51,047 | 305 | Table-free; four global bindings and field bounds included |
| Composable exact-carry modular verifier | 31,278 | 267 | Table-free; q/r bound locally; requires two adjacent certified operand vectors |
...
73-prime bases. Both are now dominated by the checked 15,626-byte
per-coordinate hybrid.

The one-shot hybrid spends 392 bytes pushing its 12 selected tables and 153
bytes destructively cleaning them up; its remaining 15,081 bytes are
...
versus 93,756 for six independent fragments.
...
five-vector no-carry modular verifier measured 52,048 bytes, 512 bytes larger
than two independent 25,768-byte proofs: deeper routing erased its 897-byte
...
performs that work from four shared 16-limb values: 38,796 of its 51,047 bytes
...
9,773-byte one-value binder when the same unsigned-256 value survives across
operations. That plain binder does not prove a field bound; the
`bind_value_below(N)` variant costs 9,860 bytes.

The 46-prime composable verifier makes certificate reuse a concrete fragment:
its matching 9,832-byte binder proves one field value and returns only its
residue certificate, while each 31,278-byte gate binds q/r and returns a new
