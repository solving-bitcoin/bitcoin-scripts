//! Full honest publication for the conditional round-major anchored candidate.
...
fn authorized_code(taus: &[Vec<u8>]) -> ScriptBuf {
    let body = body(taus, THRESHOLD, ROUNDS);
...
fn contexts(code: &ScriptBuf) -> Vec<ScriptBuf> {
    ...
    assert_eq!(out.len(), 1 + THRESHOLD * (ROUNDS + 1));
    out
}
        ...
        for j in 0..N {
            ...
            let candidate = loop {
                let b = sha256::Hash::hash(
                    format!("round-major-publication-v1-{i}-{j}-{attempt}").as_bytes(),
                )
                .to_byte_array();
                attempt += 1;
                attempts += 1;
                ...
                compact[..32].copy_from_slice(&target[1..]);
                compact[63] = 1;
                ...
                break Candidate {
                    secret,
                    r: BigUint::from_bytes_be(&target[1..]),
                    target,
                    tau,
                };
            };
...
fn verify_setup(pools: &[Pool], workers: usize) {
    ...
    for pool in pools {
        assert_eq!(pool.candidates.len(), N);
            ...
            .map(|worker| {
                scope.spawn(move || {
                    for pool in pools.iter().skip(worker).step_by(workers) {
                        for candidate in &pool.candidates {
                            assert_eq!(candidate.target.len(), 33);
                            assert_eq!(candidate.tau.len(), 40);
                            ...
                            assert_ne!(
                                hash160::Hash::hash(&candidate.tau).to_byte_array()[0],
                                0x30
                            );
                            let compact = Signature::from_der(&candidate.tau[..39])
                                .unwrap()
                                .serialize_compact();
                            assert_eq!(&compact[..32], &target[1..]);
                            assert!(compact[32..63].iter().all(|&b| b == 0));
                            assert_eq!(compact[63], 1);
                        }
...
fn main() {
    ...
    if let Some(samples) = arg("--benchmark") {
        ...
        println!("{}",serde_json::to_string_pretty(&json!({"scope":"Point-label generation, candidate/table generation, policy compilation and public checking for the entire95pool instance. Excludes garbling/VSS, transaction funding, opening generation, process startup and compilation of this executable.",
    ...
    let records = 'opening: loop {
        ...
        for (i, pool) in pools.iter().enumerate() {
            ...
            for &selected in &selections[i] {
                let pos = remaining.iter().position(|&v| v == selected).unwrap();
                let depth = remaining.len() - pos;
                remaining.remove(pos);
                depths.insert(selected, depth);
                consumption.extend([pool.candidates[selected].tau.clone(), number(depth)]);
            }
            let mut keys = vec![];
            let mut all_signatures = vec![];
            for (slot, &selected) in selections[i].iter().rev().enumerate() {
                let candidate = &pool.candidates[selected];
                let c = 1 + slot;
                let anchor_digest =
                    digest(&spending, index, &pool.checks[c], EcdsaSighashType::All);
                let z0 = BigUint::from_bytes_be(&anchor_digest) % &order;
                let t = BigUint::from_bytes_be(&candidate.secret.secret_bytes());
                let p =
                    ((&t + &order - &z0) * candidate.r.modpow(&(&order - 2u8), &order)) % &order;
                ...
                secp.verify_ecdsa(
                    &Message::from_digest(anchor_digest),
                    &Signature::from_der(&candidate.tau[..39]).unwrap(),
                    &key,
                )
                .unwrap();
                ...
                for round in 0..ROUNDS {
                    let code = &pool.checks[1 + THRESHOLD + round * THRESHOLD + slot];
            ...
            for round in 0..ROUNDS {
                for signatures in &all_signatures {
                    consumption.push(signatures[round].clone());
                }
            }
            consumption.extend(keys);
            consumption.reverse();
    ...
    let mut w: Vec<Vec<u8>> = changed.input[1]
        ...
        .map(|v| v.to_vec())
        .collect();
    let first_short = THRESHOLD * (ROUNDS + 3) - 1 - 2 * THRESHOLD;
    w[first_short - THRESHOLD] = w[first_short].clone();
    changed.input[1].witness = Witness::from_slice(&w);
    negative.push(
    ...
    println!("{}",serde_json::to_string_pretty(&json!({"scope":"Complete honest256-byte round-major publication for a candidate with an unresolved general extraction bound. No BitVM3 garbling/challenge integration is claimed.",
...
mod tests {
    ...
    #[test]
    fn sampled_and_boundary_pool_digits_roundtrip() {
        // Cover both endpoints and deterministic interior ranks; exhaustive
        // 3-million-digit testing is unnecessary for this unchanged codec.
        let radix = choose(N, THRESHOLD);
        let digits = (0..32)
            .chain(radix - 32..radix)
            .chain((0..1024).map(|i| i * (radix - 1) / 1023));
        for digit in digits {
    ...
    #[test]
    fn public_setup_checks_do_not_use_opening_secrets_and_reject_bad_public_data() {
        let (mut pools, _) = setup(1);
        for pool in &mut pools {
            for candidate in &mut pool.candidates {
                candidate.secret = one();
            }
        }
        // Public validation succeeds even when every private opening cache is
        // overwritten. This check does not prove general spend extraction.
        verify_setup(&pools, 1);
        let rejects = |bad: Vec<Pool>| {
            assert!(std::panic::catch_unwind(|| verify_setup(&bad, 1)).is_err());
        };
        let mut bad = pools.clone();
        bad.pop();
        rejects(bad);
        let mut bad = pools.clone();
        bad[0].candidates.pop();
        rejects(bad);
        let mut bad = pools.clone();
        bad[1].candidates[0] = bad[0].candidates[0].clone();
        rejects(bad);
        let mut bad = pools.clone();
        bad[0].candidates[0].tau[39] = 3;
        rejects(bad);
        let mut bad = pools.clone();
        bad[0].candidates[0].target = bad[0].candidates[1].target.clone();
        rejects(bad);
        let mut bad = pools;
        bad[0].code = ScriptBuf::new();
        rejects(bad);
    }
}
