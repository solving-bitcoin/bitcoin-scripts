...
const RADIX: usize = 230_300;
const DIGIT_BITS: usize = 18;
...
fn selected_rank_labels(
    ...
    for (i, &digit) in digits.iter().enumerate() {
        for bit in 0..DIGIT_BITS {
            selected.push(pairs[i * 19 + bit][digit >> bit & 1]);
        }
        selected.push(pairs[i * 19 + DIGIT_BITS][usize::from(invalid_flag != Some(i))]);
    }
    selected
}
...
fn check_complete_codewords(gc: &decoder::Decoder, pairs: &[[Label; 2]]) -> Value {
    ...
    json!({"cases":rows,"malformed_rank_rejected":true,"false_pool_validity_rejected":true,
        "corrupted_input_label_rejected":true,
...
pub(crate) fn run() {
    ...
    let native_path = args
        ...
        .expect("--native-recovery must name the cached anchored Core report");
    ...
    for sample in 0..samples {
        ...
        let pools = parallel(POOLS, workers, |i| Pool::new(i, N));
    ...
    println!("{}",serde_json::to_string_pretty(&json!({"evidence":"locally-reproduced","deployment":"unclassified",
        ...
        "native_report_sha256":native_hash,"encrypted_share_bytes":POOLS*N*(N-1)*16,
