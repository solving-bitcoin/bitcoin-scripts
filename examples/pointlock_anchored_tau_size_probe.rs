//! Size probe: committed long ECDSA tau anchors dynamic per-round verification keys.
...
fn taus(n:usize)->Vec<Vec<u8>>{let mut out=vec![];for i in 1..{let p=key(i);if p[1]==0||p[1]&128!=0{continue;}let mut compact=[0;64];compact[..32].copy_from_slice(&p[1..]);compact[42]=1;let mut tau=Signature::from_compact(&compact).unwrap().serialize_der().to_vec();tau.push(1);assert_eq!(tau.len(),61);out.push(tau);if out.len()==n{break;}}out}
pub(crate) fn redeem(taus:&[Vec<u8>],t:usize,d:usize,hashed:bool)->ScriptBuf{
    let n=taus.len();assert!(t>0&&t<=n&&taus.iter().all(|v|v.len()>60));
    script!{
        ...
        for j in 0..t{
            ...
            for _ in 0..d{
                {n-j} OP_ROLL OP_SIZE 60 OP_LESSTHANOREQUAL OP_VERIFY
                {n-j+1} OP_ROLL
                OP_DUP 3 OP_PICK OP_SWAP
                OP_CODESEPARATOR OP_CHECKSIGVERIFY OP_CHECKSIGVERIFY
            }
            OP_DROP
        }
...
fn measure(taus:&[Vec<u8>],s:&ScriptBuf,t:usize,d:usize,hashed:bool,m:usize)->(usize,usize,usize,u64){
    ...
    let mut items=vec![];for j in 0..t{if hashed{items.push(taus[j].clone());}items.push(num(taus.len()-j-if hashed{0}else{1}));for _ in 0..d{items.push(vec![0x30;60]);items.push(key(0));}}items.reverse();items.push(s.to_bytes());let witness=Witness::from_slice(&items);
...
fn main(){let all=taus(170);let mut rows=vec![];for hashed in [false,true]{for d in [3,4]{for n in 4..=if hashed{150}else{55}{for t in 1..=6.min(n-1){let rough=(if hashed{10}else{7}+11*d)*t+(n-t+1)/2;if rough>203{continue;}let s=redeem(&all[..n],t,d,hashed);let ops=s.instructions().filter(|v|matches!(v,Ok(bitcoin::script::Instruction::Op(op))if op.to_u8()>0x60)).count();let entry=t*(2*d+if hashed{2}else{1});if s.len()>3600||ops>201||entry>100{continue;}let m=pools(&choose(n,t));let(fv,sv,w,sw)=measure(&all[..n],&s,t,d,hashed,m);rows.push(json!({"table":if hashed{"hash160-tau"}else{"embedded-tau"},"replicas":d,"n":n,"t":t,"pools":m,"script_bytes":s.len(),"witness_bytes":w,"charged_ops":ops,"sigops":2*d*t,"hint_items":t,"entry_items":entry,"combined_stack_upper_bound":n+entry+6,"funding_vbytes":fv,"spending_vbytes":sv,"combined_vbytes":fv+sv,"spending_weight":sw}));}}}}rows.sort_by_key(|v|v["combined_vbytes"].as_u64().unwrap());println!("{}",serde_json::to_string_pretty(&json!({"scope":"Serialization and compilation only. Actual DER tau61/ALL fixtures; sigma60 and recovered public keys are placeholders. No amplification bound or native witness validity claimed.","evidence":"locally-reproduced","deployment":"unclassified","rows":rows})).unwrap());}
