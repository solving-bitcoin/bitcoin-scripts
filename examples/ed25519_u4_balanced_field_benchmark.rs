//! One-shot benchmark for balanced-radix-16 Ed25519 field multiplication.
//!
//! Run with `cargo run --release --example ed25519_u4_balanced_field_benchmark`.

use std::time::Instant;

use bitcoin::{consensus::encode::serialize, script::Instruction, Witness};
use bitcoin_lab::{
    fields::ed25519::u4_balanced_table as ed25519,
...
fn main() {
    let p = ed25519::modulus();
    // Use asymmetric, near-modulus encoded operands. In addition to stressing
    // large table entries, this deliberately exercises a nonzero reduction
    // quotient so a broken quotient/carry binding cannot hide in timing runs.
