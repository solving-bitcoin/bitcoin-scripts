//! Serialization probe: one tau-anchored key and distinct native scriptCodes.
...
pub(crate) fn redeem(taus:&[Vec<u8>],t:usize,d:usize,hashed:bool)->ScriptBuf{
    ...
    script!{
        ...
        for j in 0..t{
            ...
            for _ in 0..d{
                {n-j} OP_ROLL OP_SIZE 60 OP_EQUALVERIFY OP_OVER
...
fn input(i:usize,witness:Witness)->TxIn{let mut o=OutPoint::null();o.vout=i as u32;TxIn{previous_output:o,script_sig:ScriptBuf::new(),sequence:Sequence::MAX,witness}}
fn measure(taus:&[Vec<u8>],s:&ScriptBuf,t:usize,d:usize,hashed:bool,m:usize)->(usize,usize,usize,u64){
    ...
    let mut items=vec![];for j in 0..t{if hashed{items.push(taus[j].clone());}items.push(num(taus.len()-j-if hashed{0}else{1}));items.push(key(0));items.extend(vec![vec![0x30;60];d]);}items.reverse();items.push(s.to_bytes());let witness=Witness::from_slice(&items);
